...
that supports security features such as chroot (or unveil), privdrop (if starting
as root), fork+exec, and pledge.

unicorn-lockdown supports a few different ways of operating.  The original way
of operating, which is still supported, has the unicorn process executing as
root, and the unicorn master process continuing to run as root.  The master process
...
file system permissions), drops root privileges and starts runs as the application
user (privdrop), then runs pledge to limit the allowed system calls to the minimum
required to run the application.

unicorn-lockdown now supports an alternative way of operating, where it starts
as the application user.  The mode only supports unveil, since chroot requires
root access.  This mode still forks worker processes which re-exec (fork+exec),
then load the application, the set unveil to restrict file system access, then
pledge to limit the allowed system calls at runtime.

In both ways of operating, you can now pledge the master process, setting
permissions for the master process itself, as well as initial permissions
for the worker processes (which must be a superset of the permissions the
worker process uses).
...
with chroot (or unveil), privdrop (if starting as root), fork+exec, and pledge.
...
file for the app if one does not already exist, looking similar to:
  ...
  Unicorn.lockdown(self,
    ...
    :master_pledge=>'rpath prot_exec cpath wpath inet proc exec', # More may be needed
    :master_execpledge=>'stdio rpath prot_exec inet unix cpath wpath getpw chown unveil', # More may be needed
    :unveil => {
      'views' => 'r'
    },
    :dev_unveil => {}
...
* getpw and chown are needed to set the permissions on the log files
...
Alternatively, you can create the per-app subdirectory manually:

  doas install -d -g $app_user -o $app_user -m 700 /var/www/request-error-data/$app_name
...
You should consider adding the :master_pledge and :master_execpledge
options for additional security:

  vi /var/www/$app_name/unicorn.conf
  # add :master_pledge and :master_execpledge options
